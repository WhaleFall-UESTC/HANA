 ...
 * Original source: https://github.com/brenns10/sos/blob/master/kernel/udp.c
...
void udp_recv(struct netif *netif, struct packet *pkt)
{
	...
	/*debug("udp_recv src=%u dst=%u", ntohs(pkt->udp->src_port),
	...
	debug("nobody was waiting for this packet, freeing");
	packet_free(pkt);
}
              ...
              uint32 dst_ip, uint16 src_port, uint16 dst_port)
{
	...
	csum_add(&csum, (uint16 *)&dst_ip, 2);
	csum_add_value(&csum, ntohs((uint16)IPPROTO_UDP));
	csum_add(&csum, (uint16 *)&pkt->udp->len, 1);
	csum_add(&csum, (uint16 *)pkt->udp, len_rounded);
...
void udp_do_bind(struct socket *sock, const struct sockaddr_in *addr)
{
	...
	entry = kalloc(sizeof(struct udp_wait_entry));
	entry->sock = sock;
...
#define EPH_BEGIN 20000
	const uint16 ephemeral_begin = EPH_BEGIN;
	const uint16 ephemeral_end = 65535;

	struct sockaddr_in addr;
	static uint16 i = EPH_BEGIN;
	uint16 prev;
	uint16 tries = 100;
             ...
             socklen_t address_len)
{
	...
	if (addr.sin_addr.s_addr != 0 && addr.sin_addr.s_addr != sock->netdev->netif.ip)
...
int udp_sys_send(struct socket *sock, const void *data, size_t len, int flags)
{
	...
	if (sock->src.sin_addr.s_addr)
		...
		src = sock->netdev->netif.ip;

	udp_send(&sock->netdev->netif, pkt, src, sock->dst.sin_addr.s_addr, sock->src.sin_port,
	         sock->dst.sin_port);
	return len;
error:
	packet_free(pkt);
...
int udp_sys_recv(struct socket *sock, void *data, size_t len, int flags)
{
	...
	if ((rv = PTR_ERR(memcpy(data, pkt->al, pktlen))) < 0)
		return rv;
...
int udp_sys_close(struct socket *sock)
{
    ...
    /**
     * TODO: need further status check bufore closing the socket.
     */

    entry = lookup_entry(ntohs(sock->src.sin_port));
    if (entry) {
        error("Socket busy waiting");
        return -EBUSY;
    }

    sock->flags.sk_connected = 0;
    sock->flags.sk_bound = 0;
