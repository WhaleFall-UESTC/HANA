...
static void virtio_net_status(struct netdev *dev)
{
	...
	log("virtio_net_dev at 0x%lx",
	       virt_to_phys((uint64)netdev->regs));
	log("    Status=0x%x", READ32(netdev->regs->Status));
	log("    DeviceID=0x%x", READ32(netdev->regs->DeviceID));
	log("    VendorID=0x%x", READ32(netdev->regs->VendorID));
	log("    InterruptStatus=0x%x",
	       READ32(netdev->regs->InterruptStatus));
	log("    MagicValue=0x%x", READ32(netdev->regs->MagicValue));
	log("  tx queue:");
	log("    avail.idx = %u", netdev->tx_info->virtq.avail->idx);
	log("    used.idx = %u", netdev->tx_info->virtq.used->idx);
	WRITE32(netdev->regs->QueueSel, netdev->tx_info->queue_num);
	mb();
	log("  rx queue:");
...
int virtio_net_init(virtio_regs *regs, uint32 intid)
{
	...
	virtio_check_capabilities(regs, net_caps, nr_elem(net_caps));
