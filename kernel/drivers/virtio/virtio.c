...
struct virtqueue *virtq_create(uint32 len)
{
    ...
    /* compute offsets */
    uint32 off_desc = ALIGN(sizeof(struct virtqueue), 16);
    uint32 off_avail =
            ALIGN(off_desc + len * sizeof(struct virtqueue_desc), 2);
    uint32 off_used_event = (off_avail + sizeof(struct virtqueue_avail) +
                            len * sizeof(uint16));
    uint32 off_used = ALIGN(off_used_event + sizeof(uint16), 4);
    uint32 off_avail_event = (off_used + sizeof(struct virtqueue_used) +
                                len * sizeof(struct virtqueue_used_elem));
    uint32 off_desc_virt =
            ALIGN(off_avail_event + sizeof(uint16), sizeof(void *));
    uint32 memsize = off_desc_virt + len * sizeof(void *);

    if (memsize > PGSIZE) {
        error("virtq_create: error, too big for a page\n");
        return NULL;
    }
    page_virt = (uint32)kalloc(PGSIZE);
...
static int virtio_dev_init(uint32 virt, uint32 intid)
{
    ...
    if (READ32(regs->MagicValue) != VIRTIO_MAGIC) {
        error("error: virtio at 0x%x had wrong magic value 0x%x, "
            "expected 0x%x\n",
            virt, regs->MagicValue, VIRTIO_MAGIC);
        return -1;
    }
    if (READ32(regs->Version) != VIRTIO_VERSION) {
        error("error: virtio at 0x%x had wrong version 0x%x, expected "
            "0x%x\n",
...
{
    uint32 page_virt = VIRT_VIRTIO;

    for (int i = 0; i < 8; i++)
        virtio_dev_init(page_virt + 0x1000 * i, i);
}
